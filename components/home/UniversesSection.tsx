...
const COUNT_BADGE_TONES: Record<string, string> = {
      ...
      <div className="mx-auto max-w-screen-2xl px-4 lg:px-6">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="relative">
                ...
                return (
                  ...
                  >
                    <div className="relative z-10 flex h-full flex-col gap-3 p-4 sm:p-5">
                      <div
                        className={cn(
                          "flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl transition-all duration-300 group-hover:scale-110 sm:h-12 sm:w-12",
                          ICON_TONES[universe.color] ??
                      ...
                      <div className="min-w-0 flex-1">
                        <h3 className="text-sm font-bold leading-snug transition-colors group-hover:text-slate-950 dark:group-hover:text-white sm:text-base">
                          {universe.name}
                        </h3>
                      ...
                      <div className="flex items-center justify-between">
                        <span
                          className={cn(
                            ...
                            COUNT_BADGE_TONES[universe.color] ??
                              "bg-muted text-muted-foreground",
                          )}
                        >
                          {universe.count}
                        </span>
                        <ArrowRight className="h-3.5 w-3.5 text-muted-foreground transition-all duration-300 group-hover:translate-x-1 group-hover:text-slate-950 dark:group-hover:text-white sm:h-4 sm:w-4" />
                      </div>
                    </div>
                  </Link>
                );
            ...
            {/* Subtle gradient overlay at the bottom of the section grid */}
            <div className="pointer-events-none absolute inset-x-0 bottom-0 h-16 bg-gradient-to-t from-white/60 to-transparent dark:from-slate-950/60" />
          </div>
        </div>
      </div>
    </section>
