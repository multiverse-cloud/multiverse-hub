...
export default function HeroSection() {
  return (
    <section className="relative overflow-hidden border-b border-slate-200/70 bg-background dark:border-slate-800/80">
      ...
      <div className="absolute inset-0 bg-slate-50/50 dark:bg-slate-950/80" />
      ...
      <div className="relative mx-auto max-w-screen-2xl px-4 lg:px-6">
        <div className="mx-auto flex min-h-[calc(100svh-9rem)] max-w-4xl flex-col items-center justify-center py-8 md:py-10">
          ...
          {/* Heading */}
          <div className="mb-7 w-full text-center animate-fade-in" style={{ animationDelay: '0.1s' }}>
            <h1 className="font-display text-4xl font-extrabold leading-[0.92] tracking-tight md:text-6xl lg:text-7xl">
          ...
          {/* Suggestion chips */}
          <div className="mb-8 flex flex-wrap justify-center gap-2 animate-fade-in" style={{ animationDelay: '0.3s' }}>
            {SUGGESTION_CHIPS.map(chip => {
              ...
              return (
                <Link
                  key={chip.label}
                  href={chip.href}
                  className="flex items-center gap-1.5 rounded-full border border-border bg-card px-3 py-1.5 text-sm font-medium text-muted-foreground shadow-sm transition-all hover:border-indigo-300 hover:bg-indigo-50 hover:text-indigo-700 dark:hover:border-indigo-700 dark:hover:bg-indigo-950/40 dark:hover:text-indigo-300"
                >
          ...
          <div className="flex flex-col items-center justify-center gap-4 sm:flex-row animate-fade-in" style={{ animationDelay: '0.4s' }}>
            <Link href="/tools" className="group btn-primary flex items-center gap-2 px-8 py-3.5 text-base shadow-xl shadow-indigo-500/20 transition-shadow hover:shadow-indigo-500/30">
              <Zap className="h-5 w-5" />
              Explore All Tools
              <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" />
            </Link>
            <Link href="/tools/video/all-in-one-video-downloader" className="btn-secondary flex items-center gap-2 px-8 py-3.5 text-base">
