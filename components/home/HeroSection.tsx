...
export default function HeroSection() {
  return (
    <section className="relative border-b border-slate-200/50 bg-background dark:border-slate-800/60">
      ...
      {/* Subtle grid pattern */}
      <div
        className="absolute inset-0 grid-bg opacity-[0.15] dark:opacity-[0.08]"
        aria-hidden="true"
      />
      ...
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div
          className="absolute -right-20 top-20 h-72 w-72 rounded-full bg-indigo-500/[0.04] blur-3xl animate-float dark:bg-indigo-400/[0.06]"
          aria-hidden="true"
        />
        <div
          className="absolute -left-16 bottom-20 h-56 w-56 rounded-full bg-violet-500/[0.04] blur-3xl dark:bg-violet-400/[0.05]"
          style={{ animationDelay: "1.5s", animationDuration: "4.5s" }}
          aria-hidden="true"
        />
        <div
          className="absolute left-1/2 top-10 h-40 w-40 rounded-full bg-cyan-500/[0.03] blur-3xl dark:bg-cyan-400/[0.04]"
      ...
      <div className="relative mx-auto max-w-screen-2xl px-3 sm:px-4 lg:px-6">
        <div className="mx-auto flex min-h-[calc(100svh-6rem)] max-w-4xl flex-col items-center justify-center py-4 sm:py-6 md:py-10">
          {/* Kicker badge */}
          <div
            className="mb-3 sm:mb-4 flex justify-center animate-fade-in"
          ...
          {/* Heading */}
          <div
            className="mb-4 sm:mb-5 w-full text-center animate-fade-in px-2"
          ...
          >
            <h1 className="font-display text-2xl font-extrabold leading-[1.15] tracking-tight sm:text-3xl md:text-4xl lg:text-[3.5rem]">
              <span className="block text-slate-950 dark:text-slate-50 pb-0 sm:pb-0.5 leading-[1.15]">
                Every tool you need.
              </span>
              <span
                className="mt-0.5 sm:mt-1 block bg-gradient-to-r from-indigo-600 via-violet-500 to-purple-600 bg-clip-text text-transparent dark:from-indigo-300 dark:via-violet-300 dark:to-purple-300 leading-[1.2]"
                style={{
            ...
            <p className="mx-auto mt-3 sm:mt-4 max-w-lg text-xs leading-relaxed text-slate-600 dark:text-slate-400 sm:max-w-xl sm:text-base">
              Convert PDFs, compress images, download videos, and build
              faster&nbsp;— all free, all private, all instant.
            </p>
          </div>
          ...
          {/* Suggestion chips */}
          <div
            className="mb-4 sm:mb-6 flex flex-wrap justify-center gap-1 sm:gap-2 animate-fade-in"
          ...
          {/* CTAs */}
          <div
            className="flex flex-col items-center justify-center gap-2 sm:gap-3 sm:flex-row animate-fade-in"
            style={{ animationDelay: "0.48s", animationFillMode: "both" }}
          >
            <Link
              href="/tools"
              className="group btn-primary flex items-center justify-center gap-2 px-5 py-2.5 text-xs shadow-xl shadow-indigo-500/20 transition-all hover:shadow-indigo-500/30 hover:scale-[1.02] w-full sm:w-auto sm:px-7 sm:py-3 sm:text-base"
            >
            ...
            <Link
              href="/tools/video/all-in-one-video-downloader"
              className="btn-secondary flex items-center justify-center gap-2 px-5 py-2.5 text-xs transition-all hover:scale-[1.02] w-full sm:w-auto sm:px-7 sm:py-3 sm:text-base"
            >
