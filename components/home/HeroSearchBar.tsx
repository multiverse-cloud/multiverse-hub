    ...
    <div className="relative mb-6 w-full max-w-2xl">
      <form onSubmit={handleSubmit}>
        <div className="group relative">
          <Search className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-muted-foreground transition-colors group-focus-within:text-indigo-500" />
          <input
            type="text"
            value={query}
            onChange={event => setQuery(event.target.value)}
            placeholder="Search tools, categories, or workflows"
            className="w-full rounded-2xl border-2 border-border bg-card py-4 pl-12 pr-36 text-base shadow-lg shadow-slate-900/5 transition-all focus:border-indigo-400 focus:outline-none focus:shadow-slate-900/10 dark:shadow-none"
          />
          <button
            type="submit"
            className="absolute right-2 top-1/2 flex -translate-y-1/2 items-center gap-2 rounded-2xl bg-slate-950 px-5 py-2.5 font-display text-sm font-bold tracking-tight text-white transition-colors hover:bg-slate-800 dark:bg-slate-100 dark:text-slate-950 dark:hover:bg-white"
          >
            Search
