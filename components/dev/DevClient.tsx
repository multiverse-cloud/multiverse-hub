...
const DEV_TOOLS = [
  ...
  },
  {
    ...
    icon: LinkIcon,
    label: "URL Encoder",
    href: "/tools/dev/url-encoder-decoder",
  ...
  },
  {
    ...
    href: "/tools/dev/gitignore-generator",
    tone: "bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-200",
    tag: "new",
  },
  {
  ...
  },
  {
    ...
    icon: FileCode,
    label: "YAML to JSON",
    href: "/tools/dev/yaml-to-json-converter",
  ...
  },
  {
    ...
    icon: FileCode,
    label: "XML to JSON",
    href: "/tools/dev/xml-to-json-converter",
  ...
  },
  {
    ...
    href: "/tools/dev/markdown-to-html",
    tone: "bg-sky-100 text-sky-700 dark:bg-sky-950/40 dark:text-sky-300",
    tag: "new",
  },
  {
  ...
  },
  {
    ...
    href: "/tools/dev/qr-code-generator",
    tone: "bg-indigo-100 text-indigo-700 dark:bg-indigo-950/40 dark:text-indigo-300",
    tag: "new",
  },
  {
    ...
    href: "/tools/dev/json-schema-validator",
    tone: "bg-rose-100 text-rose-700 dark:bg-rose-950/40 dark:text-rose-300",
    tag: "new",
  },
  {
    ...
    icon: Terminal,
    label: "Docker Compose Gen",
    href: "/tools/dev/docker-compose-generator",
    tone: "bg-blue-100 text-blue-700 dark:bg-blue-950/40 dark:text-blue-300",
    tag: "beta",
  },
  {
    ...
    href: "/tools/dev/environment-manager",
    tone: "bg-teal-100 text-teal-700 dark:bg-teal-950/40 dark:text-teal-300",
    tag: "beta",
  },
];
  ...
  return (
    <div className="premium-card p-4 sm:p-5">
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        {/* Action buttons — wrap on mobile, auto width on larger screens */}
        <div className="flex flex-wrap gap-2">
          <button
            onClick={format}
            className="btn-primary w-full sm:w-auto sm:flex-none flex-1 px-3 py-2 text-xs"
          >
            Beautify
          </button>
          <button
            onClick={minify}
            className="btn-secondary w-full sm:w-auto sm:flex-none flex-1 px-3 py-2 text-xs"
          >
            Minify
          </button>
          <button
      ...
      <textarea
        ...
        rows={5}
        className="premium-textarea min-h-[100px] sm:min-h-[140px] font-mono text-xs"
      />
      {error && (
  ...
  return (
    <div className="premium-card p-4 sm:p-5">
      ...
      <textarea
        ...
        onChange={(event) => setInput(event.target.value)}
        rows={3}
        className="premium-textarea min-h-[100px] sm:min-h-[110px] font-mono text-xs"
      />
  ...
  return (
    <div className="min-h-screen premium-shell">
      <section className="page-hero">
        <div className="page-hero-inner">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <h1 className="font-display text-2xl sm:text-3xl font-extrabold tracking-tight text-slate-950 dark:text-slate-50 md:text-5xl">
              Developer Tools
            </h1>
            <p className="mx-auto mt-4 max-w-2xl text-base leading-7 text-muted-foreground">
              Format data, inspect tokens, generate IDs, and use the most
              practical utilities for daily development work.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="page-content">
        {/* Tools grid: 2 cols on mobile → 3 on sm → 4 on md → 7 on xl */}
          ...
          {DEV_TOOLS.map((tool) => {
            ...
            return (
              <Link
                key={tool.id}
                href={tool.href}
                className="premium-card group flex flex-col gap-2 p-3 sm:p-4 transition-all duration-200 hover:-translate-y-1 hover:border-slate-300 hover:shadow-lg active:scale-[0.97]"
              >
                <div
                  className={cn(
                    "flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-2xl",
                    tool.tone,
                  )}
                >
                ...
                <div>
                  <div className="mb-1 flex flex-wrap items-center gap-1">
                    <p className="text-xs sm:text-sm font-semibold leading-tight text-slate-950 dark:text-slate-50">
                      {tool.label}
                    </p>
                    {tool.tag === "hot" && (
                      <span className="tag-hot text-[10px]">Hot</span>
                    )}
                    {tool.tag === "beta" && (
                      <span className="tag-beta text-[10px]">Beta</span>
                    )}
                  </div>
                </div>
              </Link>
        ...
        <div className="mt-10">
          <div className="mb-4">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Fast inline utilities for common development tasks.
            </p>
          </div>
