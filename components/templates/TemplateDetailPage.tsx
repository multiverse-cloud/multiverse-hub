...
export default function TemplateDetailPage({
  ...
  return (
    <div className="source-hub-scope bg-slate-900">
      <main className="min-h-screen bg-slate-900 text-white">
        <header className="z-50 flex h-16 items-center justify-between bg-white px-4 shadow-sm transition-all duration-300 md:px-6">
          <div className="flex items-center gap-3 md:gap-6">
            <Link href="/templates" className="group flex items-center gap-2" title="Back to Library">
              <ArrowLeft className="h-5 w-5 text-gray-500 transition-colors group-hover:text-blue-600" />
              <span className="hidden text-sm font-semibold text-gray-700 group-hover:text-blue-600 sm:block">
                Back to Library
              </span>
            </Link>
            <div className="hidden h-6 w-px bg-gray-200 sm:block" />
            <div className="flex items-center gap-3">
              <h1 className="max-w-[120px] truncate text-sm font-bold text-gray-900 md:text-base sm:max-w-none">
                {template.title}
              </h1>
              <span className="hidden rounded bg-blue-50 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-blue-600 md:block">
                {template.industry}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 md:gap-4">
            <div className="mr-6 hidden items-center gap-2 text-gray-400 lg:flex">
              <button
                type="button"
                onClick={() => {
                  setLoaded(false)
                ...
                className={cn(
                  ...
                  viewport === 'mobile' && 'bg-blue-50 text-blue-600'
                )}
                title="Mobile Mode"
              ...
              <button
                ...
                className={cn(
                  'rounded p-2 transition-colors hover:bg-gray-100',
                  viewport === 'tablet' && 'bg-blue-50 text-blue-600'
                )}
                title="Tablet Mode"
              ...
              <button
                ...
                className={cn(
                  'rounded p-2 transition-colors hover:bg-gray-100',
                  viewport === 'desktop' && 'bg-blue-50 text-blue-600'
                )}
                title="Desktop Mode"
        ...
        <section className="relative flex h-[calc(100vh-4rem)] min-h-[720px] justify-center bg-slate-800">
          {!loaded ? (
            <div className="absolute inset-0 z-40 flex flex-col items-center justify-center space-y-4 bg-gray-900 text-white">
              <div className="h-12 w-12 animate-spin rounded-full border-4 border-blue-600 border-t-transparent" />
              <p className="animate-pulse font-medium text-gray-400">Initializing live preview...</p>
            </div>
          ) : null}
