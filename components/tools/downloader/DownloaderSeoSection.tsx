    ...
    <PremiumSection className="bg-white py-10 dark:bg-slate-950 sm:py-12">
      <PremiumContainer className="max-w-6xl">
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_340px] lg:items-start">
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                {route.howToSteps.map((step, index) => (
                  <div key={step} className="flex gap-3 rounded-lg border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-950">
                    <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-slate-950 text-xs font-black text-white dark:bg-white dark:text-slate-950">
                      {index + 1}
                    </span>
          ...
          <aside className="rounded-xl bg-slate-50 p-4 ring-1 ring-slate-200/70 dark:bg-slate-900/55 dark:ring-slate-800">
