...
export default function CompressPdfStudio({ tool }: { tool: Tool }) {
  ...
  const accept = useMemo(
    () => buildDropzoneAccept(tool.acceptedFormats),
    [tool.acceptedFormats],
  );
  const selectedProfile =
    PROFILE_OPTIONS.find((option) => option.id === profile) ||
    PROFILE_OPTIONS[1];
  const estimatedSize = file
    ? getEstimatedSize(file.size, selectedProfile.estimate)
    : null;
  const displayedSize = compressedSize ?? estimatedSize;
  const displayedSavings =
    savingsPercent ?? (file ? selectedProfile.estimate : null);
  ...
  const { getRootProps, getInputProps, isDragActive, open } = useDropzone({
    ...
    onDropRejected: (files) => {
      const tooBig = files.some((f) =>
        f.errors.some((e) => e.code === "file-too-large"),
      );
      if (tooBig) toast.error("File too large. Maximum size is 50 MB.");
      else toast.error("Invalid file. Please upload a PDF.");
    },
  });
  ...
  async function handleCompress() {
    ...
    try {
      ...
      const nextFilename = getResponseFilename(
        response,
      ...
      setSavingsPercent(
        Number(
          response.headers.get("X-Savings-Percent") || selectedProfile.estimate,
        ),
      );
      setProgress(100);
      setActivePhase(2);
  ...
  function handleDownload() {
    if (outputBlob)
      downloadBlob(outputBlob, outputFilename || "compressed.pdf");
  }
    ...
    <section className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_324px] xl:items-start sm:gap-5">
      <div className="space-y-4 sm:space-y-5">
        <div className="max-w-3xl space-y-2 sm:space-y-3">
          <span className="premium-chip text-[10px] px-2 py-0.5 sm:text-xs sm:px-2.5 sm:py-1">
            Free PDF compressor
          </span>
        ...
        >
          ...
          <button
            type="button"
            onClick={open}
            className="btn-primary min-w-32 px-6 py-2.5 text-xs sm:min-w-40 sm:px-8 sm:py-3 sm:text-sm"
          >
            Choose File
          </button>
          ...
          <p className="mt-4 text-[10px] text-slate-500 sm:mt-5 sm:text-xs">
            PDF only. Private processing.
          </p>
          {file && (
            <div className="mx-auto mt-5 flex max-w-xl items-center justify-between rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-left shadow-sm sm:mt-6 sm:rounded-2xl sm:px-4 sm:py-3">
              <div className="min-w-0">
                <p className="truncate font-display text-xs font-bold text-slate-950 sm:text-sm">
                  {file.name}
                </p>
        ...
        <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_220px] lg:items-start sm:gap-4">
            ...
            {PROFILE_OPTIONS.map((option) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    {active && (
                      <div className="h-2 w-2 rounded-full bg-indigo-600 sm:h-2.5 sm:w-2.5" />
                    )}
                  </div>
                </button>
              );
          ...
          <div className="rounded-xl border border-indigo-100 bg-indigo-50/70 p-4 sm:rounded-2xl sm:p-5">
            <h3 className="font-display text-xs font-bold tracking-tight text-indigo-700 sm:text-sm">
              Optimization Stats
            </h3>
            <div className="mt-4 space-y-4 sm:mt-5 sm:space-y-5">
              <div>
                <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-indigo-500/80 sm:text-[11px]">
                  Current Size
                </p>
      ...
      <aside className={cn("space-y-3 xl:sticky xl:top-24 sm:space-y-4", !file && !outputBlob && "hidden sm:block")}>
            ...
            <div className="space-y-1.5 sm:space-y-2">
              ...
              <p className="text-xs leading-5 text-slate-300 sm:text-sm sm:leading-6">
                {outputBlob
                  ...
                  : "Choose a preset and run compression."}
              </p>
            </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-slate-50 p-3.5 sm:rounded-2xl sm:p-5">
          <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500 sm:text-[11px]">
            Pro Tip
          </p>
          <p className="mt-1.5 text-xs leading-5 text-slate-600 sm:mt-2 sm:text-sm sm:leading-6">
            Recommended works best for most PDFs.
          </p>
        </div>

        {apiError && (
          <div className="rounded-xl border border-red-200 bg-red-50 p-3 text-xs text-red-600 sm:rounded-2xl sm:p-4 sm:text-sm">
            {apiError}
          </div>
        )}
      </aside>
