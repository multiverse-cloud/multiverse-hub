...
export default function ToolActions({ name, className }: Props) {
  ...
  return (
    <div className={cn('mb-6 flex items-center justify-end gap-1.5', className)}>
      <div className="relative">
        <button
          type="button"
          onClick={handleShare}
          className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-slate-200/80 bg-white/95 px-2.5 text-[11px] font-semibold text-slate-600 shadow-sm backdrop-blur transition-all hover:border-indigo-200 hover:text-indigo-600 hover:shadow-md dark:border-slate-800 dark:bg-slate-900/95 dark:text-slate-300 dark:hover:border-indigo-800 dark:hover:text-indigo-300"
        >
