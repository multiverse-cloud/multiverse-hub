...
const DESIGN_TOOLS = [
  { id: 'ui-generator', icon: Layout, label: 'UI Generator', desc: 'Create interface concepts and code', tone: 'bg-violet-100 text-violet-700 dark:bg-violet-950/40 dark:text-violet-300' },
  { id: 'logo-maker', icon: Sparkles, label: 'Logo Maker', desc: 'Generate brand directions', tone: 'bg-pink-100 text-pink-700 dark:bg-pink-950/40 dark:text-pink-300' },
  { id: 'poster-generator', icon: ImageIcon, label: 'Poster Generator', desc: 'Create campaign visuals', tone: 'bg-orange-100 text-orange-700 dark:bg-orange-950/40 dark:text-orange-300' },
  { id: 'landing-page', icon: Layout, label: 'Landing Page', desc: 'Draft complete page structures', tone: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300' },
  { id: 'component-gen', icon: Code2, label: 'Component Generator', desc: 'Generate React components', tone: 'bg-sky-100 text-sky-700 dark:bg-sky-950/40 dark:text-sky-300' },
  { id: 'thumbnail-gen', icon: ImageIcon, label: 'Thumbnail Generator', desc: 'Create social thumbnail directions', tone: 'bg-rose-100 text-rose-700 dark:bg-rose-950/40 dark:text-rose-300' },
]
...
export default function DesignAIClient() {
  ...
  async function generate() {
    ...
    try {
      if (activeTool === 'logo-maker' || activeTool === 'poster-generator' || activeTool === 'thumbnail-gen') {
        const prefix =
        ...
        const result = await callOpenRouter([{ role: 'user', content: prompt }], 'openai/gpt-4o-mini', systemPrompt)
        setOutput(result)
      }
    } catch {
      setOutput(
        ...
        <div className="page-hero-inner">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <p className="mx-auto mt-4 max-w-2xl text-base leading-7 text-muted-foreground">
              Generate UI concepts, component code, brand directions, posters, and social assets in one workspace.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="page-content">
        <div className="grid gap-6 lg:grid-cols-3">
          <div className="space-y-5 lg:col-span-1">
            <div>
              <p className="mb-3 text-sm font-semibold text-slate-950 dark:text-slate-50">Choose tool</p>
              <div className="grid grid-cols-2 gap-2">
                {DESIGN_TOOLS.map(item => {
                  ...
                  return (
                    ...
                    >
                      <div className={cn('mb-3 flex h-8 w-8 items-center justify-center rounded-xl', item.tone)}>
            ...
            <div className="space-y-2">
              <label className="text-sm font-semibold text-slate-950 dark:text-slate-50">Prompt</label>
              <textarea
                value={prompt}
            ...
            <div className="space-y-2">
              ...
              {examples.map(example => (
                <button
                  ...
                  className="w-full rounded-2xl border border-slate-200 bg-white px-3 py-3 text-left text-sm text-slate-600 transition-colors hover:border-slate-300 hover:text-slate-900 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300 dark:hover:border-slate-700 dark:hover:text-slate-100"
                >
                  {example}
                </button>
              ))}
            ...
            <button
              ...
              className="btn-primary w-full justify-center gap-2 disabled:pointer-events-none disabled:opacity-50"
            >
          ...
          <div className="lg:col-span-2">
            <div className="premium-card min-h-[520px] overflow-hidden">
              ...
              <div className="p-4">
                {!output && !imageUrl && !loading && (
                  <div className="flex min-h-[440px] flex-col items-center justify-center text-center">
                    <div className={cn('flex h-16 w-16 items-center justify-center rounded-3xl', tool.tone)}>
                      <tool.icon className="h-8 w-8" />
                    </div>
                    <h3 className="mt-5 font-display text-xl font-bold text-slate-950 dark:text-slate-50">{tool.label}</h3>
                    <p className="mt-2 max-w-sm text-sm leading-6 text-muted-foreground">{tool.desc}</p>
                  </div>
                )}
                ...
                {loading && (
                  <div className="flex min-h-[440px] flex-col items-center justify-center text-center">
                    ...
                    <p className="mt-4 text-sm text-muted-foreground">Generating your result</p>
                  </div>
                )}
                ...
                {output && !imageUrl && !loading && (
                  tab === 'code' ? (
                    ...
                    <div className="custom-scrollbar max-h-[460px] overflow-auto rounded-2xl border border-border bg-slate-50 p-4 dark:bg-slate-950/60">
                      <pre className="text-sm whitespace-pre-wrap leading-6 text-slate-700 dark:text-slate-200">{output}</pre>
                    </div>
                  )
                )}
              </div>
