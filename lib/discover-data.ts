import {
  HIGH_TRAFFIC_DISCOVER_GUIDE_SEEDS,
  HIGH_TRAFFIC_DISCOVER_RANKING_SEEDS,
  type DiscoverGuideSeed,
  type DiscoverRankingSeed,
  type DiscoverSeedPick,
} from './discover-high-traffic-seeds'

export type DiscoverContentType = 'ranking' | 'guide'
...
export interface DiscoverItem {
  ...
  bestFor: string
  pricing?: string
  badge: 'internal' | 'external' | 'editorial'
  url?: string
  tags: string[]
  rating?: number
}

export interface DiscoverFaq {
  question: string
  answer: string
}
...
type DiscoverListOverride = Partial<
  Pick<DiscoverList, 'seoTitle' | 'metaDescription' | 'description' | 'intro' | 'methodology' | 'faq'>
> & {
  itemSummaryOverrides?: Record<string, string>
}
...
export const DISCOVER_SCALE_PLAN = {
  starterLists: 160,
  targetLists: 2200,
  targetEntities: 5000,
  targetGuides: 1600,
...
function toDiscoverSlug(value: string) {
  return value
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '')
...
function inferDiscoverScope(...parts: Array<string | string[] | undefined>) {
...
function buildScopeSentence(scope?: string) {
  return scope ? ` Scope: ${scope}.` : ''
}

function getEditorialLens(category: string) {
...
function getGuideIntentFrame(title: string) {
...
function getGuideLensCategory(seed: DiscoverGuideSeed) {
  ...
  if (joined.includes('game')) return 'Games'
...
const DISCOVER_LIST_OVERRIDES: Record<string, DiscoverListOverride> = {
  'best-ai-tools-for-daily-work': {
    seoTitle: 'Best AI Tools for Daily Work in 2026',
    metaDescription:
      'Compare the best AI tools for daily work across drafting, research, summaries, and workflow fit without generic hype.',
    description: 'Practical AI tools ranked by how useful they feel in real daily work.',
    intro:
      'This page is built for professionals who want AI to reduce friction in normal work, not just look impressive in demos. The ranking favors tools that help with drafting, research, structured thinking, and repeatable workflow support without creating a second layer of busywork.',
    methodology: [
    ...
    faq: [
      {
        ...
        answer:
          'For most people, the safest starting point is the tool that handles drafting, summarizing, and back-and-forth editing well enough to become a default. This page helps you compare that default-tool fit more than one-time flashy outputs.',
      },
      {
        question: 'Should I use one AI tool or multiple tools?',
        answer:
          'Most people work better with one strong default tool and one backup specialist. Too many overlapping AI apps usually create more friction than leverage.',
      },
      {
        question: 'How should I compare AI tools for work?',
        answer:
          'Test them on the work you already repeat: writing, meetings, research, analysis, or planning. The better tool is the one that saves cleaner time with less correction overhead.',
      },
      {
        question: 'What mistake do people make when choosing work AI tools?',
        answer:
          'They often compare tools on fun prompts instead of real work. A good daily-work tool should feel reliable across ordinary tasks, not just impressive in one lucky example.',
      },
    ],
    itemSummaryOverrides: {
      ChatGPT:
        'ChatGPT remains one of the safest starting points for daily work because it handles drafting, brainstorming, rewriting, and explanation tasks with relatively low friction. It is strongest when you need one flexible default rather than a narrow specialist.',
      Claude:
        'Claude stands out when you need calmer long-form editing, structured reasoning, and cleaner document revision. It tends to work well for people who want a more deliberate writing and analysis companion.',
      Perplexity:
        'Perplexity is most useful when the real bottleneck is fast research, source discovery, or quick fact-pattern orientation. It often wins when you need research-style support more than pure drafting help.',
    },
  },
  'best-apps-for-earning-money': {
    seoTitle: 'Best Apps for Earning Money Without Hype',
    metaDescription:
      'A practical ranking of money-making apps based on realistic entry paths, effort required, and whether the opportunity actually fits beginners.',
    description: 'Money-making apps ranked by realistic entry path, effort, and practical usefulness.',
    intro:
      'Most people searching for earning apps are not looking for fantasy income claims. They want realistic entry points, cleaner effort-to-reward tradeoffs, and a better sense of what actually fits their time, skills, and patience. This ranking is built with that lens.',
    methodology: [
    ...
    faq: [
      {
        ...
        answer:
          'Yes, but the better question is what kind of work the app unlocks. The strongest options usually help you sell a service, product, or skill rather than magically generate income with no effort.',
      },
      {
        question: 'What is the safest type of earning app to start with?',
        answer:
          'Apps tied to real work, like freelancing, selling, tutoring, or creator workflows, are usually more reliable than gimmicky reward systems with unclear payout quality.',
      },
      {
        question: 'How should I pick a money-making app?',
        answer:
          'Match the app to your actual asset: time, skill, audience, products, or communication ability. The best app is the one that fits what you can do consistently.',
      },
      {
        question: 'What mistake should beginners avoid?',
        answer:
          'Avoid choosing an app only because it promises fast money. Look for clarity: how the money is made, who pays, what work is involved, and how repeatable the process really is.',
      },
    ],
  },
  'best-online-jobs-for-students-without-confusion': {
    ...
    metaDescription:
      'Compare realistic online jobs for students based on schedule fit, skill barrier, and how safely they work alongside studies.',
    description: 'Student-friendly online jobs ranked by schedule fit, skill barrier, and realistic earning path.',
    intro:
      'Students do not need a giant list of random online jobs. They need options that fit around classes, have a believable entry path, and do not destroy focus or sleep. This page favors jobs that feel more realistic when studies still need to come first.',
    methodology: [
  ...
  'best-startup-ideas-for-solo-founders': {
    ...
    metaDescription:
      'A solo-founder startup ideas ranking focused on low coordination overhead, realistic execution, and clearer validation paths.',
    description: 'Solo-founder startup ideas ranked by execution simplicity and validation potential.',
    intro:
      'Solo founders do not need the broadest idea. They need the idea that can survive limited bandwidth, lower coordination capacity, and slower early momentum. This page focuses on ideas that still make sense when one person is doing the thinking, building, selling, and iterating.',
    methodology: [
  ...
  'best-productivity-apps-for-students-in-2026': {
    ...
    metaDescription:
      'Compare the best productivity apps for students across deadlines, notes, focus, and revision without building an overloaded tool stack.',
    description: 'Student productivity apps ranked by deadline clarity, focus support, and study workflow fit.',
    intro:
      'Students usually do not need more apps. They need fewer apps that make deadlines, notes, focus sessions, and revision easier to manage. This ranking favors tools that reduce chaos instead of expanding it.',
    methodology: [
  ...
  'best-apps-for-personal-finance-tracking': {
    ...
    metaDescription:
      'A practical ranking of personal finance tracking apps focused on spending clarity, budgeting visibility, and monthly control.',
    description: 'Finance tracking apps ranked by spending clarity, budgeting visibility, and ease of review.',
    intro:
      'A good finance tracking app should make your money feel more legible. The right choice is usually the one that helps you notice patterns, control categories, and actually review your spending without turning budgeting into a second full-time job.',
    methodology: [
  ...
  'how-to-choose-the-right-ai-tool-for-your-workflow': {
    ...
    metaDescription:
      'A practical guide to choosing the right AI tool by task pattern, workflow friction, output quality, and real daily use.',
    description: 'A practical framework for choosing AI tools by real workflow fit instead of hype.',
    intro:
      'Most AI tool confusion comes from shopping by feature list instead of by repeated task. This guide is built for people who feel buried under too many AI choices and want a cleaner way to decide what actually belongs in their workflow.',
    methodology: [
    ...
    faq: [
      {
        ...
        answer:
          'Compare them on your real repeated tasks, not on novelty prompts. The better tool is the one that stays useful when the task becomes ordinary, messy, or slightly harder.',
      },
      {
        question: 'Should I keep multiple AI tools?',
        answer:
          'Usually one default tool and one backup specialist is enough. More than that often creates decision drag unless your workflow is unusually broad.',
      },
      {
        question: 'What matters more: raw quality or workflow fit?',
        answer:
          'Workflow fit often wins. A slightly smarter tool can still be the wrong daily choice if it slows you down, adds friction, or forces awkward switching.',
      },
      {
        question: 'What mistake should I avoid while choosing?',
        answer:
          'Do not choose based only on hype, leaderboard talk, or one clever answer. The real test is whether the tool improves your repeated work without adding cleanup burden.',
      },
    ],
  },
  'how-to-start-freelancing-as-a-student': {
    ...
    metaDescription:
      'A student-first freelancing guide focused on manageable service selection, proof of work, and protecting study time.',
    description: 'A student-first freelancing guide built around manageable services, proof of work, and schedule safety.',
    intro:
      'Freelancing can help students earn and build real-world skill, but it turns risky fast when it starts competing with classes, sleep, or revision. This guide is designed to help students start smaller, build proof carefully, and avoid chaotic overcommitment.',
    methodology: [
  ...
  'what-is-product-market-fit-in-simple-terms': {
    ...
    metaDescription:
      'A plain-language guide to product-market fit for beginners who want to understand startup growth without jargon overload.',
    description: 'A plain-language explanation of product-market fit for people who want startup ideas to make more sense.',
    intro:
      'Product-market fit gets discussed like a magic milestone, which makes the concept harder than it needs to be. This guide turns it into a simpler mental model: what demand feels like, how usage changes, and why retention matters more than compliments.',
    methodology: [
  ...
  'how-to-build-better-study-notes': {
    ...
    metaDescription:
      'A practical study-notes guide focused on faster capture, clearer structure, and revision-friendly note quality.',
    description: 'A practical study-notes guide for students who want revision to feel easier and less messy.',
    intro:
      'Good notes are not just neat notes. They should help you recall faster, spot confusion earlier, and reduce revision panic later. This guide focuses on turning class material into notes that stay useful when exams get closer.',
    methodology: [
  ...
  'how-to-fix-phone-battery-drain-without-panic': {
    ...
    metaDescription:
      'A practical phone battery-drain guide covering settings, apps, signal issues, and battery-age reality before expensive replacement.',
    description: 'A practical battery-drain guide for people whose phone dies too fast and needs a calmer troubleshooting path.',
    intro:
      'Battery drain often feels mysterious because several small issues can combine into one bad experience. This guide is designed to help you inspect the real cause first, fix the biggest drains, and decide whether the phone needs tuning or the battery is simply aging out.',
    methodology: [
  ...
  'how-to-understand-geopolitics-without-getting-lost-in-news': {
    ...
    metaDescription:
      'A beginner-friendly geopolitics guide that turns noisy headlines into clearer geography, incentives, strategy, and context.',
    description: 'A beginner-friendly geopolitics guide for turning noisy headlines into clearer strategic context.',
    intro:
      'Geopolitics feels overwhelming when every headline seems disconnected from the last one. This guide is built to make world events more coherent by using geography, incentives, regional focus, and longer strategic patterns instead of random headline surfing.',
    methodology: [
  ...
  'how-to-start-learning-about-space-as-a-beginner': {
    ...
    metaDescription:
      'A beginner-friendly space guide built around missions, stories, visuals, and curiosity before technical overload.',
    description: 'A beginner-friendly space guide designed to build curiosity before technical overwhelm.',
    intro:
      'Space becomes easier to learn when it starts with missions, stories, and visible scale instead of abstract technical overload. This guide is designed to help beginners build curiosity, orientation, and momentum before they move into denser science.',
    methodology: [
  ...
  'how-to-follow-politics-more-intelligently': {
    ...
    metaDescription:
      'A practical politics guide focused on systems, source quality, policy patterns, and calmer long-term understanding.',
    description: 'A practical politics guide for people who want better understanding without nonstop overload.',
    intro:
      'Following politics becomes less exhausting when you stop treating every controversy as equally important. This guide is designed to help readers build a calmer, smarter information habit centered on systems, source quality, and policy patterns rather than endless outrage cycles.',
    methodology: [
  ...
  'how-to-choose-better-productivity-apps': {
    ...
    metaDescription:
      'A practical guide to choosing productivity apps based on real workflow bottlenecks, simpler stacks, and lower daily friction.',
    description: 'A practical productivity-app guide built around real bottlenecks, lower friction, and simpler tool stacks.',
    intro:
      'Most productivity problems do not come from a lack of apps. They come from too many overlapping tools, unclear bottlenecks, and stacks that look impressive but are hard to maintain. This guide helps readers choose tools based on actual work patterns instead of app hype.',
    methodology: [
...
function buildSeedSummary(seed: DiscoverRankingSeed, pick: DiscoverSeedPick) {
  const lens = getEditorialLens(seed.category)
  const tagPhrase = pick.tags.slice(0, 2).join(' and ')
  return `${pick.name} stands out if you want ${lowerFirst(
    pick.bestFor
  )}. It earns its place through ${tagPhrase} and a stronger fit for ${seed.subcategory.toLowerCase()} readers who care about ${lens.summaryFocus}.`
}

function buildRankingMethodology(seed: DiscoverRankingSeed) {
  const lens = getEditorialLens(seed.category)
  return [
...
function buildGuideMethodology(seed: DiscoverGuideSeed) {
...
function buildGeneratedRankingFaq(seed: DiscoverRankingSeed): DiscoverFaq[] {
  const lens = getEditorialLens(seed.category)
  ...
  return [
    ...
    },
    {
      ...
      answer: firstPick
        ? `${firstPick.name} is usually the cleanest starting point if you want ${lowerFirst(
            firstPick.bestFor
          )}, then you can move down the list if your priorities are narrower.`
        : `Use the shortlist as a fit-first decision page instead of assuming the first slot is the only good option.`,
    },
  ]
}

function buildGeneratedGuideFaq(seed: DiscoverGuideSeed): DiscoverFaq[] {
  const lens = getEditorialLens(getGuideLensCategory(seed))
  const frame = getGuideIntentFrame(seed.title)
...
function buildGeneratedRankingList(seed: DiscoverRankingSeed): DiscoverList {
  ...
  return {
    ...
    metaDescription: `Explore ${seed.title.toLowerCase()} curated for ${lowerFirst(
      seed.audience
    )}. These editorial picks focus on ${seed.angle}.${buildScopeSentence(scope)}`,
    description: `${seed.title} curated for ${lowerFirst(seed.audience)}.${buildScopeSentence(scope)}`,
    intro: `This ranking is built for ${lowerFirst(seed.audience)}. We prioritized ${seed.angle}, replay value, and how confidently each pick can be recommended when someone wants better ${seed.subcategory.toLowerCase()} options.${buildScopeSentence(
      scope
    )}`,
...
function buildGeneratedGuideList(seed: DiscoverGuideSeed): DiscoverList {
  const slug = toDiscoverSlug(seed.title)
  const scope = seed.scope || inferDiscoverScope(seed.title, seed.tags)
  ...
  return {
    ...
    metaDescription: `Learn ${seed.title.toLowerCase()} with a practical step-by-step path built for ${lowerFirst(
      seed.audience
    )}.${buildScopeSentence(scope)}`,
    description: `${seed.title} for ${lowerFirst(seed.audience)}.${buildScopeSentence(scope)}`,
    intro: `This guide is built for ${lowerFirst(seed.audience)}. The goal is ${seed.angle}, so each step is written to remove friction and make the next decision easier.${buildScopeSentence(
      scope
    )}`,
...
function withDerivedScope(lists: DiscoverList[]) {
...
function withAutoRelatedSlugs(lists: DiscoverList[]) {
  return lists.map(list => {
...
function assertUniqueDiscoverLists(lists: DiscoverList[]) {
  ...
  for (const list of lists) {
    ...
    if (duplicateIdTitle) {
      throw new Error(
        `Duplicate discover id "${list.id}" found for "${duplicateIdTitle}" and "${list.title}".`
      )
    }
    ...
    if (duplicateSlugTitle) {
      throw new Error(
        `Duplicate discover slug "${list.slug}" found for "${duplicateSlugTitle}" and "${list.title}".`
      )
    }
...
const BASE_DISCOVER_LISTS: DiscoverList[] = [
  {
...
const GENERATED_DISCOVER_LISTS: DiscoverList[] = [
  ...HIGH_TRAFFIC_DISCOVER_RANKING_SEEDS.map(buildGeneratedRankingList),
  ...HIGH_TRAFFIC_DISCOVER_GUIDE_SEEDS.map(buildGeneratedGuideList),
].map(applyDiscoverListOverride)

export const DISCOVER_LISTS: DiscoverList[] = assertUniqueDiscoverLists(
  withAutoRelatedSlugs(withDerivedScope([...BASE_DISCOVER_LISTS, ...GENERATED_DISCOVER_LISTS]))
)

export function getPublishedDiscoverLists() {
  return DISCOVER_LISTS.filter(list => list.published)
}

export function getDiscoverListBySlugLocal(slug: string) {
  return DISCOVER_LISTS.find(list => list.slug === slug) || null
}
