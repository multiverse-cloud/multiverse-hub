...
import { Compass, LayoutTemplate, Search, ShieldAlert, Sparkles, Wrench } from 'lucide-react'
...
export const metadata: Metadata = {
  ...
  description: 'Search across tools, discover pages, fixes, prompts, and UI templates in one place.',
}
...
export default async function SearchPage({ searchParams }: SearchPageProps) {
  ...
  const allResults = query.length >= 2 ? await searchSiteContent(query, 36) : []
  const results = activeType === 'all' ? allResults : allResults.filter(result => result.type === activeType)
  const availableTypes = Array.from(new Set(allResults.map(result => result.type)))
      ...
      <div className="page-content space-y-8 pt-10">
        <section className="rounded-3xl border border-border bg-card p-6 md:p-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h1 className="text-2xl font-bold md:text-3xl">Search Across The Multiverse</h1>
            </div>
          </div>
          <p className="mt-3 max-w-3xl text-sm leading-7 text-muted-foreground">
            Search tools, discover rankings and guides, fixes, prompts, and UI templates from one result surface.
          </p>
        ...
        {query.length < 2 ? (
          <div className="rounded-2xl border border-dashed border-border bg-card px-6 py-12 text-center">
            <p className="text-base font-semibold">Enter at least 2 characters to search.</p>
            <p className="mt-2 text-sm text-muted-foreground">
              Try searches like "AI tools", "phone battery", "pricing page", or "mobile banking UI".
            </p>
          </div>
        ) : results.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-border bg-card px-6 py-12 text-center">
            <p className="text-base font-semibold">No matches found for "{query}".</p>
            <p className="mt-2 text-sm text-muted-foreground">
          ...
          <section className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Showing <span className="font-semibold text-foreground">{results.length}</span> results for{' '}
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="mt-2 text-base font-bold">{result.title}</p>
                      </div>
                    </div>
                    <p className="mt-3 text-sm leading-6 text-muted-foreground">{result.description}</p>
