  ...
  return (
    <div className="max-w-screen-lg space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="flex items-center gap-2 text-2xl font-bold"><Flag className="h-5 w-5 text-orange-500" /> Feature Flags</h1>
          <p className="mt-1 text-sm text-muted-foreground">Enable or disable platform features without redeploying</p>
        </div>
            ...
            {flags.filter(f => f.group === group).map(flag => (
              <div key={flag.id} className="flex items-center justify-between px-5 py-4">
                <div className="mr-4 min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 text-xs text-muted-foreground">{flag.desc}</p>
                  <code className="mt-0.5 block font-mono text-xs text-indigo-600 dark:text-indigo-400">{flag.id}</code>
                </div>
