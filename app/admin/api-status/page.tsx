...
export default function APIStatusPage() {
  ...
  return (
    <div className="max-w-screen-lg space-y-5">
      ...
      <div>
        <h1 className="flex items-center gap-2 text-2xl font-extrabold sm:text-3xl text-slate-950 dark:text-slate-50">
          <Activity className="h-6 w-6 text-violet-500" />
          API Status
        </h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Monitor and configure external integrations.
        </p>
      </div>
...
NEXT_PUBLIC_GNEWS_API_KEY=xxxx`}</pre>
      ...
      <div className="space-y-3">
        {APIS.map((api) => {
          const isBuiltIn =
            api.key.startsWith("Free") || api.key.startsWith("Built-in");
